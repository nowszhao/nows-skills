...
cp -R skills/nows-keywordfilter ~/.skills/
```
...
| `nows-keywordfilter` | 对文本做敏感词/违禁词过滤与替换：调用 skill 内 bin/ 目录预编译的跨平台 `keywordfilter` 二进制（Go + Aho-Corasick 多模式匹配），词表已编译进二进制内部，运行时不依赖外部文件 | 发布前做内容审核脱敏、批量过滤文本里的敏感/违禁词、想看命中数量时 | 替换后文本（命中字符按长度填充 `-replace` 字符，默认 `*`），`-stats` 在 stderr 输出命中数 | `帮我把这段文字过滤一下敏感词`<br>`这段文案做个违禁词替换`<br>`check 一下这段有没有敏感内容`<br>`filter 这段` |
