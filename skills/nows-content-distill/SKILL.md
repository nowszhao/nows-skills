...
- **必做**：针对话题领域，用 web_search 检索 3+ 个其他主流观点 / 同类内容，建立"默认认知"基线，供 Step 4 的"差异化亮点"使用
- **按需做**：
...
每次经过 2 次及以上尝试才搞定的问题（抓取失败、金字塔分层不稳、HTML 渲染走样等），追加到 `LESSONS.md`，下一次先看这份。
