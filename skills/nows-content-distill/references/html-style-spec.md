...
## 一·五、最易走样点（实测高发问题，重点防）

下面这些是上次实际生成时出过的偏离，每条都要逐项核对：

1. **H2 标题不要改成短色条 + inline-block**：必须是**全宽细线下划线**（`border-bottom: 1px solid var(--rule); padding-bottom: 12px;`），不是 `border-bottom: 2px solid var(--accent); display: inline-block;`
2. **差异化亮点必须用 `.diff-item` 包每一条**：每条独立成块，含 `<h3>` 标题 + `<div class="diff-pair">` 左右对照 + `<p class="diff-why">` 该条独立的"为什么有价值"。**禁止把所有"多数人会说" / "这一篇说" 平铺成一个大 grid**
3. **每条差异化亮点要有自己的"为什么有价值"**：不是全篇共用一段 diff-why 总评
4. **视频金句必须含 `<span class="source">`**：格式 `视频 [HH:MM]` 或 `视频 [MM:SS]`，无时间戳即不合格
5. **最简复述里禁止 `<strong>` / `<em>` / `<b>` / `<mark>`**：连贯散文意味着不能有视觉强调
6. **ASCII 金字塔图必须方框宽度统一、连线对齐**：在浏览器打开实际看一眼，不是写完就交
7. **footer 自检数字必须真实**：写"信心不足 0 处"时，MD 必须有非空 `## 调研记录` 段落且 ≥3 同生态来源
