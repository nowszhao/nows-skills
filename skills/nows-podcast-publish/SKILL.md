...
description: 将音频一键发布到小宇宙播客平台。覆盖全流程：通过 bmx CLI 获取/处理音频 → YouTube 溯源获取原视频链接与封面 → 基于字幕内容提炼吸引眼球的标题（3 候选让用户选）→ AI 生成单集介绍与章节导航 → 浏览器自动化登录小宇宙后台上传并发布。当用户提到「发布播客」「上传到小宇宙」「播客单集发布」「nows podcast publish」时使用。
---
...
**标题是播客的「点击率第一道关卡」，不能简单翻译音频文件名，必须基于字幕全文提炼。**

流程：

1. **加载 Prompt**：从 `references/content_prompts.md` 加载「标题生成 Prompt」
2. **生成候选**：将字幕文本替换 `{SUBTITLE_TEXT}` 后，让模型通读字幕、分析核心爆点、生成 **3 个候选标题**
3. **展示候选并让用户选择**：用 `AskUserQuestion` 展示 3 个候选，让用户从中选择或要求修改
...
**标题硬性要求：**
- 提炼冲突/反转/洞察，不准只是「XX 主题分享」
- 信息密度高：暗示谁、什么事、为什么值得听
- 20-35 字，在小宇宙里不被截断
- 必须来自字幕真实内容，禁止标题党（编造字幕里没有的观点）

**用户确认后才进入 Phase 3.2**。若用户对所有候选都不满意，根据反馈重新生成。用户也可以直接口述标题。
...
将**标题、原文链接、单集介绍、章节导航**完整展示给用户预览。用 `AskUserQuestion` 确认：

- 标题是否满意？不满意可以回 3.1 重新生成或用户直接口述
- 单集介绍是否准确？有没有曲解原文的地方？
- 章节导航划分和时间戳是否合理？简介是否太短？

**全确认后才进入 Phase 4。** 标题和章节导航是最容易返工的环节，不要急着发。
...
| ✅5 | Phase 3.1 | 从 3 个候选标题中选择或要求重新生成 |
| ✅6 | Phase 3.6 | 确认标题、单集介绍、章节导航完整内容 |
| ✅7 | Phase 4.2 | 小宇宙已登录 |
| ✅8 | Phase 4.10 | 发布成功截图 |
| ✅9 | Phase 4.10.5 | 回编辑页校验封面已正确写入（`hasImg:true` 且为新 `xyzcdn` 图，非默认图） |
